...
export const errorHandler = {
  ...
  createError: (message, code = ERROR_CODES.UNKNOWN_ERROR, context = {}) => {
    return new ServiceError(message, code, 500, context);
  },

  // Enhanced error handling with detailed context
  handle: (error, operation = '', additionalContext = {}) => {
    const errorContext = {
      operation,
  ...
  async: (asyncFn, operation = '', fallbackValue = null, shouldThrow = true) => {
    return async (...args) => {
      try {
        return await asyncFn(...args);
      } catch (error) {
        const handled = errorHandler.handle(error, operation, { args });
